<template>
    <footer class="footer">
        <div class="footer__wrap">
            <div class="footer__content">
                <div class="footer__col footer__col--a">
                    <div class="footer__logo">
                        <svg version="1.1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 995.6 326.6"
                            xml:space="preserve" width="125" height="65">
                            <path fill="#ffffff" d="M974.5,117l5.1,1.9c0.8,0.3,0.8,1.8,0,2.1l-5.1,1.9c-20.8,7.8-36.5,31.2-40.2,60.1l-5.2,40.4
		c-0.1,1.2-1.4,1.2-1.5,0l-5.2-40.4c-3.7-28.9-19.4-52.3-40.2-60.1l-5.1-1.9c-0.8-0.3-0.8-1.8,0-2.1l5.1-1.9
		c20.8-7.8,36.5-31.2,40.2-60.1l5.2-40.4c0.2-1.2,1.4-1.2,1.5,0l5.2,40.4C938,85.8,953.7,109.2,974.5,117" />
                            <path fill="#ffffff" d="M447,215.6l48.7-81.5c1.4-2.1-0.1-4.9-2.6-4.9h-48.1c-1,0-2,0.5-2.6,1.4l-23.1,38.6l-23.1-38.6
		c-0.6-0.9-1.6-1.4-2.6-1.4h-48.1c-2.5,0-4,2.8-2.6,4.9l48.7,81.5l-48.7,81.5c-1.4,2.1,0.1,4.9,2.6,4.9h48.1c1,0,2-0.5,2.6-1.4
		l23.1-38.6l23.1,38.6c0.6,0.9,1.6,1.4,2.6,1.4H493c2.5,0,4-2.8,2.6-4.9L447,215.6z" />
                            <path fill="#ffffff" d="M112.1,129.1H23.3c-1.5,0-2.7,1.2-2.7,2.7v167.4c0,1.5,1.2,2.7,2.7,2.7h88.7c47.7,0,86.4-28.4,86.4-86.4
		C198.5,157.5,159.8,129.1,112.1,129.1z M110.1,256.7H71.1v-78.6h39.1c20.3,0,36.8,13.8,36.8,39.3
		C146.9,242.9,130.4,256.7,110.1,256.7z" />
                            <path fill="#ffffff" d="M768.8,129.1l-101.4,0c-0.6,0-1.1,0.5-1.1,1.1v170.6c0,0.6,0.5,1.1,1.1,1.1h48.2c0.6,0,1.1-0.5,1.1-1.1V179.1
		h52.1c0.6,0,1.1-0.5,1.1-1.1v-47.7C769.9,129.6,769.4,129.1,768.8,129.1z" />
                            <path fill="#ffffff" d="M653.5,248.8c-0.1-0.4-0.2-0.7-0.4-1.1c0,0,0-0.1,0-0.1c0,0,0,0,0,0c-5.3-14.9-17.2-26.5-32.3-30.8l-4.4-1.2
		c-0.7-0.2-0.7-1.2,0-1.3l4.4-1.2c15.6-4.4,27.8-16.6,32.8-32.2c0.3-1.1,0.4-2,0.4-2.7v-46.2c0-1.5-1.2-2.8-2.7-2.8h-45
		c-1.5,0-2.7,1.2-2.7,2.8v10.2c-9.7-9.1-23.1-14.2-41-14.2c-47.7,0-86.4,28.7-86.4,87.4s38.7,87.4,86.4,87.4
		c17.9,0,31.3-5.1,41-14.2v10.8c0,1.5,1.2,2.8,2.7,2.8h45c1.5,0,2.7-1.2,2.7-2.8v-47.5C653.9,251,653.8,250,653.5,248.8z
		 M556.7,256.8c-20.7,0-37.5-14.5-37.5-41.5s16.8-41.5,37.5-41.5c20.7,0,37.5,14.5,37.5,41.5S577.4,256.8,556.7,256.8z" />
                            <path fill="#ffffff" d="M361.7,193.5c-8.8-39-41.3-65.6-83.6-65.6c-32.8,0-59.7,16-74.2,41.3c-0.3,0.6-1.3,2.7-0.8,5
		c4.3,17.2,17.2,30.9,34,35.6l4.4,1.2c0.7,0.2,0.7,1.2,0,1.3l-4.4,1.2c-18,5.1-32.5,20.3-34.8,39.4c0,0-0.2,4.6,0.6,6.2
		c14.2,26.5,41.7,43.3,75.3,43.3c37.8,0,67.7-21.2,80-53.5c0.8-2-0.7-4.1-2.8-4.1h-46.4c-0.8,0-1.6,0.3-2.1,0.9
		c-6.9,6.9-16.7,10.8-28.6,10.8c-16.3,0-28.7-7.3-35.1-19.7h70.3h46c1.4,0,2.7-1,3-2.4c1.2-6.2,1.9-12.6,1.9-19.4
		C364,207.7,363.2,200.4,361.7,193.5z M242.9,193.5c6.4-12.4,18.8-19.7,35.1-19.7c16.3,0,28.7,7.3,35.1,19.7H242.9z" />
                            <path fill="#ffffff" d="M849.4,243.8c0,4.6-3.8,8.4-8.4,8.4c-4.6,0-8.4-3.8-8.4-8.4v-17.9h62c0.6,0,1.1-0.5,1.1-1.1v-48.1
		c0-0.6-0.5-1.1-1.1-1.1h-62l0-45.1c0-0.7-0.5-1.2-1.1-1.2h-48.2c-0.6,0-1.1,0.5-1.1,1.2v113.5c0,32.4,26.4,58.7,58.8,58.7
		c32.4,0,58.8-26.4,58.8-58.7H849.4z" />
                        </svg>
                    </div>

                    <div class="footer__copyright">Dexart (c) 2022</div>

                    <Socials class="footer__socials" />
                </div>

                <div class="footer__col footer__col--b">
                    <div class="footer__title">{{ t('info') }}</div>

                    <nav class="footer__footer-menu footer-menu">
                        <ul class="footer-menu__list">
                            <li class="footer-menu__item" v-for="item in menu1" :key="item.label">
                                <a :href="item.url" target="_blank" class="gooter-menu__link"
                                    v-if="item.url.includes('http')">
                                    {{ item.label }}
                                </a>
                                <RouterLink class="header-menu__link" active-class="footer-menu__link--active"
                                    :to="item.url" v-else>
                                    {{ item.label }}
                                </RouterLink>
                            </li>
                        </ul>
                    </nav>
                </div>

                <div class="footer__col footer__col--c">
                    <div class="footer__title">{{ t('sitemap') }}</div>

                    <nav class="footer__footer-menu footer-menu">
                        <ul class="footer-menu__list">
                            <li class="footer-menu__item" v-for="item in menu2" :key="item.label">
                                <a :href="item.url" target="_blank" class="gooter-menu__link"
                                    v-if="item.url.includes('http')">
                                    {{ item.label }}
                                </a>
                                <RouterLink class="header-menu__link" active-class="footer-menu__link--active"
                                    :to="item.url" v-else>
                                    {{ item.label }}
                                </RouterLink>
                            </li>
                        </ul>
                    </nav>
                </div>

                <div class="footer__col footer__col--d">
                    <SubscriptionForm class="footer__subscription-form" />

                    <div class="footer__disclaimer">
                        No Investment Advice
                        The information provided on this website is for information purpose only, you should not
                        construe any such information or other material as investment advice, financial advice, trading
                        advice, or any other sort of advice and you should not treat any of the website's content as
                        such. Nothing contained on DEXART website constitutes a solicitation, recommendation,
                        endorsement, or offer by DEXART that any cryptocurrency should be bought, sold, or held by you.
                        Do conduct your own due diligence and consult your financial advisor before making any
                        investment decisions
                    </div>
                </div>
            </div>

        </div>
    </footer>
</template>

<script setup lang="ts">
import { computed } from 'vue';
import { useI18n } from 'vue-i18n';
import Socials from './Socials.vue'
import SubscriptionForm from './SubscriptionForm.vue'

const { t, locale } = useI18n({
    messages: {
        en: {
            heading: 'Press about <span>DEXART</span>',
            btn: 'Learn More',
            info: 'Info',
            sitemap: 'Sitemap'
        },
        ru: {
            heading: 'Пресса о <span>Dexart</span>',
            btn: 'Подробнее',
            info: 'Инфо',
            sitemap: 'Карта сайта'
        }
    }
})

const menu1 = computed(() => {
    if (locale.value === 'ru') {
        return [
            {
                label: 'WhitePaper',
                url: '/empty',
            },
            {
                label: 'Политика конфиденциальности',
                url: 'https://drive.google.com/file/d/1VbEB6wefa5TqqX-acHxyikX1FrC657dN/view',
            },
            {
                label: 'Политика в отношении cookie',
                url: 'https://drive.google.com/file/d/1AURUWSwUAsIdNRTJ-dcgHZKNcbiXF04e/view',
            },
            {
                label: 'Пользовательское соглашение',
                url: 'https://drive.google.com/file/d/1MKC63zHN1AcJMhuiv5d4_y6tpmfY6XLV/view',
            },
        ]
    }

    return [
        {
            label: 'WhitePaper',
            url: '/empty',
        },
        {
            label: 'Privacy Policy',
            url: 'https://drive.google.com/file/d/1VbEB6wefa5TqqX-acHxyikX1FrC657dN/view',
        },
        {
            label: 'Cookies Policy',
            url: 'https://drive.google.com/file/d/1AURUWSwUAsIdNRTJ-dcgHZKNcbiXF04e/view',
        },
        {
            label: 'Term of service',
            url: 'https://drive.google.com/file/d/1MKC63zHN1AcJMhuiv5d4_y6tpmfY6XLV/view',
        },
    ]
})

const menu2 = computed(() => {
    if (locale.value === 'ru') {
        return [
            {
                label: 'Главная',
                url: 'https://dex.art ',
            },
            {
                label: 'Комната аватара',
                url: '',
            },
            {
                label: 'История мира',
                url: '/history',
            },
            {
                label: 'Токен',
                url: '',
            },
        ]
    }

    return [
        {
            label: 'Main',
            url: 'https://dex.art',
        },
        {
            label: 'Avatar Hub',
            url: '',
        },
        {
            label: 'World backstory',
            url: '/history',
        },
        {
            label: 'Token',
            url: '',
        },
    ]
})

</script>

<style scoped lang="scss">
.footer {
    &__wrap {
        background: linear-gradient(180deg, #160d1f -4.22%, #140c1b 100%);
        font-family: 'StyreneAWeb', sans-serif;
        padding-top: rem(80px);
        padding-bottom: rem(80px);
        position: relative;
        z-index: 10;

        @include media-breakpoint-down(md) {
            padding: rem(64px 0);
        }
    }

    &__content {
        display: grid;
        grid-template-columns: 3fr 2fr 2fr 5fr;
        grid-template-areas: 'a b c d';
        gap: rem(16px);
        width: calc(100% - 64px);
        max-width: 1472px;
        margin: auto;

        @include media-breakpoint-down(lg) {
            grid-template-columns: 1fr 1fr;
            gap: rem(32px);
            grid-template-areas:
                'a d'
                'b c';
        }

        @include media-breakpoint-down(md) {
            gap: rem(40px);
            grid-template-columns: 1fr;

            grid-template-areas:
                'a'
                'd'
                'b'
                'c';
        }
    }

    &__col {
        &--a {
            grid-area: a;
        }

        &--b {
            grid-area: b;
        }

        &--c {
            grid-area: c;
        }

        &--d {
            grid-area: d;
        }
    }

    &__disclaimer {
        font-size: rem(10px);
        margin-top: rem(30px);
        color: rgba(#fff, 0.4);
        font-weight: 300;

        @include media-breakpoint-down(md) {
            padding-bottom: rem(32px);
            border-bottom: 1px solid;
            border-image-source: linear-gradient(rgba(#ece7fa, 0), #ece7fa);
        }
    }

    &__logo {
        margin-bottom: rem(16px);

        @include media-breakpoint-down(md) {
            margin-bottom: 2px;
        }

        img {
            max-width: 125px;

            @include media-breakpoint-down(md) {
                max-width: 95px;
            }
        }
    }

    &__copyright {
        font-weight: 300;
        font-size: rem(18px);
        line-height: 120%;
        color: rgba(#ffffff, 0.8);
        margin-bottom: rem(28px);

        @include media-breakpoint-down(md) {
            font-size: rem(14px);
            margin-bottom: rem(16px);
        }
    }

    &__title {
        font-weight: 700;
        font-size: rem(20px);
        line-height: 130%;
        margin-bottom: rem(28px);
        letter-spacing: 0.01em;
        color: #ffffff;

        @include media-breakpoint-down(md) {
            margin-bottom: rem(16px);
            font-weight: 900;
        }
    }


    &__created-by {
        font-size: rem(14px);
        color: #4f5775;
        text-align: right;
        width: calc(100% - 64px);

        @include media-breakpoint-down(md) {
            text-align: left;
            margin: auto;
            margin-top: rem(32px);
        }

        a {
            color: #912eef;
            text-decoration: none;
        }
    }
}

.footer-menu {
    &__list {
        @include unlist;
        display: grid;
        grid-template-columns: 1fr;
        gap: rem(24px);
    }

    &__link {
        font-weight: 300;
        font-size: rem(18px);
        line-height: 120%;
        color: #ffffff;
        text-decoration: none;

        @include media-breakpoint-down(md) {
            font-size: rem(16px);
        }

        &:active {
            color: #912eef;
        }
    }
}
</style>
